...
import { ModelProps, QueryRowFormat } from "@itwin/core-common";
import { GeometricModel2dState, GeometricModel3dState, IModelApp, IModelConnection } from "@itwin/core-frontend";
...
import { ChangedElementEntry, ChangeElementType } from "./ChangedElementEntryCache";
...
/**
 * Provider for changed elements tree that contains even unchanged elements. The roots of the tree are all the changed
 * top assembly (elements without parents) found in the changes.
 */
export class ChangesTreeDataProvider implements ITreeDataProvider {
  ...
  constructor(private _manager: VersionCompareManager) {
    this._rootElements = new Set(
      this._manager.changedElementsManager.entryCache
        .getAll()
        .filter((value) => value.elementType !== undefined && value.elementType === ChangeElementType.TopAssembly)
        .map((value: ChangedElementEntry) => value.id),
    );
    if (this._manager.currentIModel) {
      this._currentLabelProvider = new PresentationLabelsProvider({ imodel: this._manager.currentIModel });
    }

    if (this._manager.targetIModel) {
      this._targetLabelProvider = new PresentationLabelsProvider({ imodel: this._manager.targetIModel });
    }
  }

  private get _elements() {
    return this._manager.changedElementsManager.entryCache.changedElementEntries;
  }
  ...
  private _splitEntries(entries: ChangedElementEntry[], splitSize: number): ChangedElementEntry[][] {
  ...
  public isSearchingInBackground = (): boolean => {
    return this._isProcessingSearch;
  };

  private _backgroundSearchLoad = async (): Promise<void> => {
  ...
  public numberOfChangedElementsToLoad(): number {
    let count = 0;
    for (const pair of this._elements) {
      const element = pair[1];
      if (!this._isLoaded(element)) {
  ...
  private _getNode = (element: ChangedElementEntry): DelayLoadedTreeNodeItem => {
    const label = element.label !== undefined && element.label !== ""
      ? element.label
  ...
  private async _getUnchangedNodes(elementIds: string[]): Promise<ImmediatelyLoadedTreeNodeItem[]> {
    const keys = elementIds.map((id) => ({ id, className: "BisCore:Element" }));
    const labels = this._currentLabelProvider !== undefined ? await this._currentLabelProvider.getLabels(keys) : [];
  ...
  public async getNodesCount(parent?: TreeNodeItem | undefined): Promise<number> {
    if (parent === undefined) {
      if (this._manager.currentIModel === undefined || this._manager.targetIModel === undefined) {
        return 0;
      }

      const nodes = await this._getAllChangedModelNodes(this._manager.currentIModel, this._manager.targetIModel);
      return nodes.length;
    }
  ...
  private _getAllChildrenOfNodes = (nodes: TreeNodeItem[]) => {
    ...
    for (const child of nodes) {
      ...
      if (child.extendedData && child.extendedData.element.children?.length > 0) {
  ...
  private _createNodeFromModel(
    ...
    const childrenEntries = childrenNodes.map((node) => node.extendedData?.element.id ?? "");
  ...
  /**
   * Find which model ids provided match the given class name. Also enforces to only find public models.
  ...
  private _getModelsOfClass = async (
    ...
    const models = modelIds.reduce((a, b) => a + "," + b);
    const ecsql = "SELECT ECInstanceId FROM " + className + " WHERE ECInstanceId in (" + models + ")"
      + (filterIsPrivate ? " AND IsPrivate = false" : "");
    const response = iModel.query(ecsql, undefined, { rowFormat: QueryRowFormat.UseJsPropertyNames });
  ...
  private _getModels3dInArray = async (iModel: IModelConnection, modelIds: string[]): Promise<string[]> => {
    return this._getModelsOfClass(iModel, modelIds, GeometricModel3dState.classFullName, true);
  };

  private _getModels2dInArray = async (iModel: IModelConnection, modelIds: string[]): Promise<string[]> => {
    return this._getModelsOfClass(iModel, modelIds, GeometricModel2dState.classFullName, true);
  };
  ...
  private async _getAllChangedModelNodes(
    ...
    const onlyDeleted = (entry: ChangedElementEntry) => entry.opcode === Opcode.Delete;
    const notDeleted = (entry: ChangedElementEntry) => entry.opcode !== Opcode.Delete;

    const currentNodes = this._manager.reparentModelNodes
      ? await this._getChangedModelOrSubjectNodes(currentIModel, notDeleted, false, updateFunc)
      : await this._getChangedModelNodes(currentIModel, notDeleted, false, updateFunc);

    let targetNodes = this._manager.reparentModelNodes
      ? await this._getChangedModelOrSubjectNodes(targetIModel, onlyDeleted, true, updateFunc)
      : await this._getChangedModelNodes(targetIModel, onlyDeleted, true, updateFunc);

    const currentModelIds = new Set(currentNodes.map((node) => node.id));
    targetNodes = targetNodes.filter((entry) => !currentModelIds.has(entry.id));
   ...
   */
  private async _getModelLabels(props: ModelProps[], current: boolean): Promise<Map<string, string>> {
    const labelMap = new Map<string, string>();
    try {
      const labels = await (current ? this._currentLabelProvider : this._targetLabelProvider)?.getLabels(
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        props.map((prop) => ({ id: prop.id!, className: prop.classFullName! })),
      );
      if (labels) {
      ...
      Logger.logWarning(VersionCompare.logCategory, `Could not load labels for models: ${error}`);
    }
  ...
  /**
   * Finds all the relevant model ids of the top node/root entry elements that are relevant in the comparison session.
   */
  ...
  private _createModelNodes(
    ...
    for (const prop of props) {
      ...
      // Only add the node related to either all models or the specific 2d model being viewed
  ...
  private async _loadParentModelLabels(
    ...
    await this._loadParentModelLabelsOfClass(labelMap, iModel, GeometricModel3dState.classFullName, current, true);
    await this._loadParentModelLabelsOfClass(labelMap, iModel, GeometricModel2dState.classFullName, current, true);
    await this._loadParentModelLabelsOfClass(labelMap, iModel, "BisCore:Subject", current, false);
  }
  ...
  private async _getChangedModelOrSubjectNodes(
    ...
    if (this._manager.changedElementsManager.modelToParentModelMap) {
      for (const pair of this._manager.changedElementsManager.modelToParentModelMap) {
    ...
    // Find the 3D models
    const found3dModels = new Set(await this._getModels3dInArray(iModel, props.map((prop) => prop.id ?? "")));
    ...
    // Find the 2D models
    const found2dModels = new Set(await this._getModels2dInArray(iModel, props.map((prop) => prop.id ?? "")));
    ...
    const nodes = this._createModelNodes(props, labelMap, found3dModels, found2dModels, markAsDelete);
    // TODO: Remove duplicates and handle multi-nodes
    return nodes.sort((a, b) => a.label.property.displayLabel.localeCompare(b.label.property.displayLabel));
  }

  /** Get relevant changed models for comparison. */
  private async _getChangedModelNodes(
    iModel: IModelConnection,
    filterFunc?: (entry: ChangedElementEntry) => boolean,
    markAsDelete?: boolean,
    updateFunc?: () => void,
    ...
    // Create the model nodes from all the necessary data
    return this._createModelNodes(props, labelMap, found3dModels, found2dModels, markAsDelete);
  }

  private _getChangedModelChildrenById(modelId: string): TreeNodeItem[] {
    return this.getEntriesFromIds(this._rootElements)
      .filter((entry) => entry.modelId !== undefined && entry.modelId === modelId)
      .map((entry) => this._getNode(entry));
  }

  private _getChangedModelChildrenCount(modelNode: TreeNodeItem): number {
    if (modelNode.extendedData === undefined || modelNode.extendedData.modelId === undefined) {
      return 0;
    }
  ...
  private async _getChangedModelChildren(modelNode: TreeNodeItem): Promise<TreeNodeItem[]> {
    if (modelNode.extendedData?.modelId === undefined || modelNode.extendedData?.childrenEntries === undefined) {
      return [];
    }
  ...
  public setSearch = (search: string | undefined): void => {
    if (search === "" || search === undefined) {
      ...
      this._backgroundSearchLoad().catch(() => { /* No-op */ });
    }
  };
  ...
  private _matches = (search: string, label: string): boolean => {
  ...
  private _getSearchResultNodes = async (search: string): Promise<TreeNodeItem[]> => {
    ...
    const modelNodes = await this._getAllChangedModelNodes(this._manager.currentIModel, this._manager.targetIModel);
    // Filter model nodes
    const filteredModels = modelNodes.filter((node) => {
      const primVal = node.label.value;
      if (primVal === undefined) {
        return false;
      }

      const value = (primVal as PrimitiveValue).displayValue ?? ((primVal as PrimitiveValue).value as string);
      if (value === undefined) {
        return false;
      }
  ...
  public load = async (nodes: TreeNodeItem[]): Promise<void> => {
    ...
    const entries = ids
      ...
      .filter((entry) => entry !== undefined) as ChangedElementEntry[];
    await this._loadEntries(entries);
  };

  private _loadEntries = async (entries: ChangedElementEntry[]): Promise<void> => {
    // Load entries in the entry cache and load their children
    await this._manager.changedElementsManager.entryCache.loadEntries(entries, true);
    // Raise event that new nodes where updated/added to the elements result so that components may re-load and force
    // updates
  ...
  public async getNodes(parent?: TreeNodeItem | undefined): Promise<DelayLoadedTreeNodeItem[]> {
    ...
    if (parent === undefined) {
      return this._getAllChangedModelNodes(this._manager.currentIModel, this._manager.targetIModel);
    }
    ...
    if (this._elements.has(parent.id)) {
      ...
      if (entry?.directChildren !== undefined && entry?.directChildren?.length !== 0) {
