 ...
 */
export class ChangedElementsInspector extends React.Component<ChangedElementsInspectorProps> {
  public override render() {
    if (
      this.props.manager.changedElementsManager.entryCache.dataProvider ===
      undefined
    ) {
      throw new Error(
...
class ChangedElementsBreadCrumb extends React.Component<BreadCrumbProps> {
  private _breadcrumbEndRef = React.createRef<HTMLDivElement>();
    ...
    this.props.path.forEach((node: TreeNodeItem) => {
      const label =
        typeof node.label === "string"
          ? node.label
          : (node.label.value as PrimitiveValue).displayValue;
      menuItems.push(
        <MenuItem
          key={node.id}
          onClick={() => {
        ...
        {this.props.path.length > 0 && (
          <Breadcrumbs className="vc-itwinui-breadcrumb-container">
            <DropdownButton
              ...
              menuItems={this.dropdownMenuItems}
            />
            {lastNode && (
              <Button
                styleType="borderless"
                onClick={() => this.props.pathClicked(lastNode)}
              >
                {getLabel(lastNode)}
              </Button>
            )}
...
const ChangeTypeFilterHeader = (props: FilterHeaderProps) => {
  ...
  const openAdvancedFilterDialog = async () => {
    try {
      ...
      const properties = data.map((filter: PropertyFilter) => {
        return {
      ...
      const finalData = data
        .map((filter: PropertyFilter) => {
          return {
            ...filter,
            label:
              PropertyLabelCache.getLabel(filter.classId, filter.name) ??
              filter.name,
          };
        })
        .sort((a: PropertyFilter, b: PropertyFilter) => {
          return b.label?.localeCompare(a.label ?? "") ?? 0;
        });
      ...
      Logger.logError(
        ...
        "Advanced Dialog Opening Error: " + e,
      );
      props.onLoadLabels?.(true);
    }
  };
    ...
    const makeContextMenuItem = (localeStr: string, flag: number) => {
      ...
      return (
        <Checkbox
          ...
          onClick={() => {
            const opts = props.options;
            opts.wantedTypeOfChange = isOn
              ? opts.wantedTypeOfChange & ~flag
              : opts.wantedTypeOfChange | flag;
            props.onFilterChange(opts);
          }}
        />
    ...
    return (
      <>
        ...
        {props.wantPropertyFiltering && (
          <div className="vc-context-menu-separator" />
        )}
        {props.wantPropertyFiltering && (
          <MenuItem
...
export class ChangedElementsListComponent extends React.Component<
  ChangedElementsListProps,
  ChangedElementsListState
> {
  ...
  constructor(props: ChangedElementsListProps) {
    ...
    const propertyNames =
      props.manager.changedElementsManager.getAllChangedPropertyNames();
  ...
  public static cleanMaintainedState = () => {
  ...
  private _handleSearchUpdate = async () => {
  ...
  public loadState = async () => {
    ...
    if (mState) {
      ...
      const visualizationManager =
        this.props.manager.visualization?.getSingleViewVisualizationManager();
  ...
  private onHandleShowAll = async () => {
    await this.props.onShowAll();
    this._refreshVisibility();
  };

  private onHandleHideAll = async () => {
    await this.props.onHideAll();
    this._refreshVisibility();
  };

  private onInvert = async () => {
    await this.props.onInvert();
    this._refreshVisibility();
  };

  /** Add listeners */
  public attachToViewport = async (vp: ScreenViewport) => {
    if (vp) {
      this._attachedVp = vp;
      ...
      if (filtered) {
        ...
        if (baseModelId !== undefined) {
          focusNode = nodes.find((node: TreeNodeItem) => {
            return (
              node.extendedData && node.extendedData.modelId === baseModelId
            );
          });
        }
  ...
  /** Remove listeners */
  public dettachFromViewport = (vp: ScreenViewport) => {
    if (vp) {
  ...
  private _handlePathClick = async (
    node: TreeNodeItem | undefined,
    reset?: boolean,
  ) => {
    ...
    const index = path.findIndex((tmpNode: TreeNodeItem) => {
      return tmpNode.id === node.id;
    });
  ...
  private _isModelNode = (node: TreeNodeItem) => {
    return node.extendedData?.isModel;
  };

  private _areModelNodes = (nodes: TreeNodeItem[]) => {
  ...
  private _isElementNode = (node: TreeNodeItem) => {
    return (
      node.extendedData !== undefined && node.extendedData.isModel === undefined
    );
  };
  ...
  private _mergeEntryArrays = (
    a: ChangedElementEntry[],
    b: ChangedElementEntry[],
  ): ChangedElementEntry[] => {
    return [...a, ...b];
  };

  /** Obtains the children nodes of the models, creates their entries, and visualizes them */
  private _visualizeModelNodes = async (
    nodes: TreeNodeItem[],
    options?: FilterOptions,
  ) => {
    // Handle model nodes: Get the children entries they already have into an array
    const entries = nodes
      .map((node: TreeNodeItem) => this.props.dataProvider.getModelAllChildElementEntries(node.id))
      .reduce(this._mergeEntryArrays, []);

    const filter = options
      ? (entry: ChangedElementEntry) =>
        this._filterEntryWithGivenOptions(entry, options)
  ...
  private _anyEntryPropertiesVisible = (
    entry: ChangedElementEntry,
    options: FilterOptions,
  ): boolean => {
   ...
   */
  private _modifiedEntryMatchesFilters = (
    entry: ChangedElementEntry,
    options: FilterOptions,
  ): boolean => {
    ...
    // Treat an indirect element as invalid for the filter matching, they will be shown
    // due to their child elements wanted to get show, not because they contain specific
    // modified changes that match the filters
  ...
  private _wantShowEntry = (
    ...
    return (
      ...
      (options.wantModified &&
        entry.opcode === DbOpcode.Update &&
        this._modifiedEntryMatchesFilters(entry, options)) ||
  ...
  private _childrenWantShow = (
    parent: ChangedElementEntry,
    options: FilterOptions,
  ): boolean => {
    ...
    const entries =
      this.props.manager.changedElementsManager.entryCache
        .changedElementEntries;
    for (const child of children) {
      const entry = entries.get(child);
      if (
        entry !== undefined &&
        (this._wantShowEntry(entry, options) ||
          this._childrenWantShow(entry, options))
      ) {
        return true;
      }
    }
  ...
  private _wantShowNode = (
    ...
    const opcode =
      node.extendedData.element !== undefined
        ? node.extendedData.element.opcode
        : undefined;
  ...
  public getFilteredNodes = (nodes: TreeNodeItem[], options: FilterOptions) => {
    // Nothing to filter
    ...
    return nodes.filter((node: TreeNodeItem) => this._wantShowNode(node, options));
  };

  /** Update filter options and update visualization */
  public handleFilterChange = async (options: FilterOptions) => {
    // Get filtered nodes and update visualization
  ...
  private _handleClearSearch = async () => {
  ...
  private async _getCurrentNodesFromProvider() {
    const currentNode = this._getCurrentPathNode();
    const nodes = await this.props.dataProvider.getNodes(currentNode);
    return nodes;
  }
  ...
  /** Loads the search */
  private _loadSearch = async (search?: string) => {
  ...
  private _loadNodes = async (nodes: TreeNodeItem[]) => {
    await this.props.dataProvider.load(nodes);
    await this._reloadNodes();
  };

  private _isNodeLoaded = (node: TreeNodeItem) => {
    return this.props.dataProvider.isLoaded(node);
  };

  private _isSelected = (item: TreeNodeItem) => {
    return this.state.selectedIds.has(item.id);
  };

  private _hasChildren = (item: DelayLoadedTreeNodeItem) => {
    return (
      item.hasChildren ??
      (item.extendedData?.isModel
        ? this._modelNodeHasChangedChildren(item)
        : this._nodeHasChangedChildren(item))
    );
  };

  private _isItemVisible = (item: TreeNodeItem): boolean => {
    const elementIdsNeededForVisibility =
      this.props.dataProvider.getRelatedElementIds(item);
    ...
    return item.extendedData?.isModel
      ? visualizationManager.isModelVisibile(
        item.id,
        item.extendedData?.element.opcode === DbOpcode.Delete,
      )
      : visualizationManager.isAnyVisible(new Set(elementIdsNeededForVisibility));
  };

  private _toggleVisibility = async (item: TreeNodeItem) => {
    const elementsNeededForVisibility =
      this.props.dataProvider.getRelatedElements(item);
    ...
    if (visualizationManager) {
      if (item.extendedData?.isModel) {
        ...
        await visualizationManager.toggleChangedElementsVisibility(
          !isVisible,
          elementsNeededForVisibility,
        );
      } else {
        await visualizationManager.toggleUnchangedElementVisibility(
          !isVisible,
          item.id,
        );
      }
  ...
  private _onInspect = async (item: TreeNodeItem) => {
      ...
      .then(() => {
        this.props.dataProvider
          ...
          .then(async (nodes: DelayLoadedTreeNodeItem[]) => {
            ...
            if (
              item.extendedData?.isModel &&
              this.props.manager.options.onModelInspect
            ) {
              this.props.manager.options.onModelInspect(
                item.extendedData.modelProps,
                item.extendedData.is2d,
              );
            }
   ...
   */
  private _onNodeClick = async (item: TreeNodeItem) => {
    const visualizationManager =
      this.props.manager.visualization?.getSingleViewVisualizationManager();
  ...
  private _onPropertyCompare = async () => {
    this.saveState();
    ...
    return (
      <div className="element-list-container">
        ...
        <ChangeTypeFilterHeader
          ...
          wantPropertyFiltering={
            this.props.manager.wantPropertyFiltering &&
            this.props.manager.wantNinezone
          }
