...
import {
  SelectionMode, Table, TableDataChangeEvent, TableSelectionTarget, type ColumnDescription, type ItemColorOverrides,
  type PropertyCategory, type PropertyData, type RowItem, type TableDataProvider
...
export class OverviewOpacitySlider extends React.Component<OverviewOpacitySliderProps> {
...
/**
 * Property Comparison Table data provider
 * Consists of two data providers that connect to the current IModelConnection
 * and the target IModelConnection to obtain properties for an element from two different
 * backends with different versions of the iModel. It also provides flattening out those
 * properties to easily render them in a table and it merges the results into a single
 * structure
 */
export class PropertyComparisonTableDataProvider implements TableDataProvider {
  ...
  private _primaryVersionDataProvider:
    | PresentationPropertyDataProvider
    | undefined;
  private _secondaryVersionDataProvider:
    | PresentationPropertyDataProvider
    | undefined;
  ...
  private _comparisonData: Map<string, ComparisonDataRow> = new Map<
    string,
    ComparisonDataRow
  >();
  ...
  private _getFlattenedProperties(data: PropertyData): SingleComparisonData[] {
    ...
    const processCategory = ({
      ...
      if (data.records[name]) {
        data.records[name].forEach((value: PropertyRecord) => {
          this._processPropertyRecord(rows, label, value);
        });
      } else {
        Logger.logWarning(
          LOGGER_CATEGORY,
          `No records found with category name: ${name}`,
        );
      }
      if (childCategories) {
    ...
    data.categories.forEach((category: PropertyCategory) => {
   ...
   */
  private _manufactureKey(value: SingleComparisonData) {
    return value.category + "." + value.propertyName;
  }

  /** Inserts data into the row map */
  private _primaryDataChangedHandler = async () => {
    ...
    const rows: SingleComparisonData[] = this._primaryData
      ? this._getFlattenedProperties(this._primaryData)
      : [];
    if (this._comparisonData) {
      ...
      for (const value of rows) {
        ...
        if (this._comparisonData.has(key)) {
          ...
          const entry: ComparisonDataRow | undefined =
            this._comparisonData.get(key);
          if (entry) {
  ...
  /** Inserts data into the row map */
  private _secondaryDataChangedHandler = async () => {
    ...
    const rows: SingleComparisonData[] = this._secondaryData
      ? this._getFlattenedProperties(this._secondaryData)
      : [];
  ...
  private _updateChangedOnly() {
    this._filteredData = new Map<string, ComparisonDataRow>();
    if (this._comparisonData !== undefined) {
      this._comparisonData.forEach((value: ComparisonDataRow, key: string) => {
        if (
   ...
   * @param selection
   */
  public async updateSelection(selection: KeySet) {
    if (
      this._primaryVersionDataProvider &&
      this._secondaryVersionDataProvider
    ) {
  ...
  /**
   * Sets up the two data providers that communicate with both IModelConnection objects to obtain properties
   * in each of the versions being inspected
  ...
  public async setupProviders(
    ...
    this._primaryVersionDataProvider = new PresentationPropertyDataProvider({
      imodel: mainConnection,
    });
    this._secondaryVersionDataProvider = new PresentationPropertyDataProvider({
      imodel: secondaryConnection,
    });
  ...
  public cleanUp() {
    ...
    if (
      this._secondaryDataChangedHandler &&
      this._secondaryVersionDataProvider
    ) {
  ...
  public async getColumns(): Promise<ColumnDescription[]> {
    if (!this._manager) {
      Logger.logError(
        VersionCompare.logCategory,
        "VersionCompareManager not defined in PropertyComparisonTable",
      );
      throw new Error(
        "VersionCompareManager not defined in PropertyComparisonTable",
      );
    ...
    let currentLabel =
      this._manager.currentVersion && this._manager.currentVersion.displayName
    ...
    const targetLabel =
      this._manager.targetVersion && this._manager.targetVersion.displayName
  ...
  public async getRowsCount(): Promise<number> {
    if (!this._comparisonData) {
      return 0;
    } else if (!this._showChangedOnly) {
      return this._comparisonData.size;
    } else if (this._filteredData) {
  ...
  private _generateRowArray(): ComparisonDataRow[] {
    if (!this._showChangedOnly && this._comparisonData) {
      ...
      this._comparisonData.forEach((value: ComparisonDataRow) => {
        array.push(value);
      });
    ...
    if (this._showChangedOnly && this._filteredData) {
      const array: ComparisonDataRow[] = [];
      this._filteredData.forEach((value: ComparisonDataRow) => {
        array.push(value);
      });
  ...
  private _createPropertyRecord(value: string, column: ColumnDescription) {
  ...
  public async getRow(_rowIndex: number): Promise<RowItem> {
    ...
    const overrides: ItemColorOverrides | undefined =
      this.getColorOverrides(row);
    return {
      ...
      cells: [
        ...
        {
          key: "propertyName",
          record: this._createPropertyRecord(
            row.propertyName,
            this._columns[1],
          ),
        },
   ...
   */
  public isRowSelected(rowItem: RowItem) {
   ...
   */
  public isChangedRow(value: ComparisonDataRow) {
  ...
  public async sort(): // _columnIndex: number,
...
// We need to pass a lambda that returns the connection so that each time we open the frontstage we get the right connections dynamically
// The Control that gets the application data only gets created once, so I haven't found a better way to decouple this component
// from the App than passing in getter functions as props.
...
export class PropertyComparisonTable extends React.Component<
  PropertyComparisonProps,
  PropertyComparisonTableState
> {
  private _unmounted = false;
  private _onPropertiesLoading = () => {
    this.safelySetState({
      ...this.state,
      loading: true,
    });
  ...
  constructor(props: PropertyComparisonProps) {
    ...
    this.state = {
      manager,
      loading: true,
      sideBySide: manager.wantNinezone
  ...
  private _updateElementInfo = async (currentSelection: KeySet) => {
    ...
    let entries =
      await this.state.manager.changedElementsManager.entryCache.get([
        selectedId,
      ]);
    if (entries.length !== 1) {
      return;
    ...
    if (this.state.manager.changedElementsManager.labels !== undefined) {
      entries =
        await this.state.manager.changedElementsManager.labels.populateEntries(entries);
    }

    const entry = entries[0];
    this.safelySetState({
      changedElement: entry,
    });
  };
   ...
   * @param provider
   */
  private _selectionChangedHandler = async (args: SelectionChangeEventArgs) => {
    // Make non-readonly
  ...
  public override async componentDidMount() {
    ...
    // If we are comparing already (e.g. ninezone mode goes into property comparison
    // frontstage when already comparing), setup data providers and load the change type of element
    if (
      this.state.manager.currentIModel !== undefined &&
      this.state.manager.targetIModel !== undefined &&
      this.state.manager.isComparing
    ) {
      await this._setupDataProvider(
        this.state.manager.currentIModel,
        this.state.manager.targetIModel,
      );
    }
  }
  ...
  private safelySetState<K extends keyof PropertyComparisonTableState>(state: Pick<PropertyComparisonTableState, K>) {
    if (!this._unmounted) {
      this.setState(state);
    }
  }

  public async handleCycle(up: boolean) {
  ...
  private _onToggleInspectMode = async () => {
    // Toggle layout
  ...
  private getHeader() {
    const setShowChanged = () => {
      ...
      VersionCompareUtils.outputVerbose(
        VersionCompareVerboseMessages.propertyComparisonTableToggleShowChangedProps,
      );
      this.safelySetState({
    ...
    const changeTypeInfo =
      this.state.changedElement !== undefined
    ...
    return (
      <div className="header">
        <div className="header-element-label">
          <div
            className="header-element-label-text"
            title={this.state.changedElement?.label}
          >
