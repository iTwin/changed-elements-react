...
/**
 * IMPORTANT: As of @bentley/version-compare@4.0.0 your application should not use the VersionCompareManager
 * any more. You should use VersionCompare.initialize. The manager will be maintained by the package.
 *
 * Main orchestrator for version compare functionality and workflows.
 * Your application should maintain a VersionCompareManager if it wants to take
 * advantage of the UI components that are maintained in this package.
 *
 * This class does the following:
 *
 * 1. Handles the Design Review focused workflow of version compare by providing
 *    difference visualization via coloring elements in the view and displaying
 *    removed elements from older versions in the current application's viewport.
 * 2. Provides Property Comparison by starting up a special frontstage bundled with
 *    the version compare functionality. This frontstage provides side-by-side comparison
 *    and a property compare table.
...
export class VersionCompareManager {
  ...
  private _initializeVisualizationHandler() {
    // Undefined defaults to true for this option
    if (this.wantNinezone && this.options.ninezoneOptions) {
      this._visualizationHandler = new NinezoneVisualizationHandler(
        this,
        this.options.ninezoneOptions,
      );
    } else if (this.options.simpleVisualizationOptions !== undefined) {
      this._visualizationHandler = new SimpleVisualizationHandler(
        this,
        this.options.simpleVisualizationOptions,
      );
    } else {
      throw new Error(
  ...
  public get wantNinezone(): boolean {
    return (
      this.options.ninezoneOptions !== undefined &&
      (this.options.wantNinezone === undefined ||
        this.options.wantNinezone === true)
    );
  }
  ...
  /**
   * Triggers when version compare startup finished successfully
   * and provides a way to obtain the current IModel,
   * the target IModel for the version compared against
   * and the changedElements retrieved from the service.
   */
  ...
  /**
   * Elements that should be ignored during initialization
   * Helpful for editing applications that may not want to compare locally changed elements
   */
  ...
  private _filterIgnoredElementsFromChangeset = (changeset: ChangedElements): ChangedElements => {
    ...
    for (let i = 0; i < changeset.elements.length; ++i) {
      if (!this.ignoredElementIds.has(changeset.elements[i])) {
        ...
        if (
          changeset.properties !== undefined &&
          i < changeset.properties.length
        ) {
  ...
  public async getChangedElements(
    ...
    const changedElements = await (client instanceof ChangedElementsApiClient &&
      changesetChunks
      ? client.getChangedElementsInChunks(projectId, iModelId, changesetChunks)
      : client.getChangedElements(
        projectId,
        iModelId,
        current.changesetId,
        target.changesetId,
   ...
   * @param onViewChanged [optional] this event serves as a communication channel to let version compare frontstage manager know about changes in categories/models/emphasize elements. The app should raise this event whenever it changes anything related to view.
   * @param changesetChunks [optional] If present, the provided chunks will be used to load changed element data
   */
  public async startComparison(
    ...
    try {
      ...
      const changedElementEntries =
        this.changedElementsManager.entryCache.getAll();
      ...
      this._hasParentIds = changedElementEntries.some(
        (entry: ChangedElementEntry) =>
          entry.parent !== undefined && entry.parentClassId !== undefined,
      );
      // We have type of change available if any of the entries has a valid type of change value
      this._hasTypeOfChange = changedElementEntries.some(
        (entry: ChangedElementEntry) => entry.type !== 0,
      );
      ...
      this._hasPropertiesForFiltering = changedElementEntries.some(
        (entry: ChangedElementEntry) =>
          entry.properties !== undefined && entry.properties.size !== 0,
      );
      ...
      await this.changedElementsManager.entryCache.initialLoad(
        changedElementEntries.map((entry: ChangedElementEntry) => entry.id),
      );
      ...
      // Raise event
      this.versionCompareStarted.raiseEvent(
        this._currentIModel,
        this._targetIModel,
      ...
      const detailed = IModelApp.localization.getLocalizedString(
        "VersionCompare:versionCompare.error_cantStart",
      );
   ...
   */
  public async enableVisualization(
  ...
  public async stopComparison() {
    ...
    try {
      ...
      Logger.logError(LOGGER_CATEGORY, "Failed to stop comparison", () => ({
        ex,
      }));
    }
