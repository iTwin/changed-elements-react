...
import { ChangedElements, Opcode } from "./api/changedElementsApi";
import { ChangedElement, ChangedElementEntryCache, Checksums } from "./ChangedElementsWidget/ChangedElementEntryCache";
...
export const hasTypeOfChange = (element: ChangedElement, toc: TypeOfChange): boolean => {
...
/**
 * Goes through all elements and cleans up the type of change in case their properties got flipped back and forth to the
 * same values.
 */
export const cleanMergedElements = (changedElements: Map<string, ChangedElement>): void => {
  ...
  for (const id of elementIds) {
    ...
    if ((hasTypeOfChange(currentElement, TypeOfChange.Property)
      || hasTypeOfChange(currentElement, TypeOfChange.Indirect))
      && (currentElement.properties === undefined || currentElement.properties.size === 0)
    ) {
    ...
    if (currentElement.opcode === Opcode.Update && currentElement.type === 0) {
...
export const accumulateChange = (
  ...
  if (forward) {
    ...
    if (
      (current.opcode === Opcode.Insert && opcode === Opcode.Update) ||
      // Support Insert + Insert - happens due to schema change + overflow tables
      (current.opcode === Opcode.Insert && opcode === Opcode.Insert)
    ) {
      return;
    }
    ...
    if (
      (current.opcode === Opcode.Update && opcode === Opcode.Update) ||
      // Support Update + Insert - happens due to schema change + overflow tables - keep as update
      (current.opcode === Opcode.Update && opcode === Opcode.Insert)
    ) {
      ...
      changedElements.set(
        elementId,
        {
          ...
          // When going forwards, the "current" value contains the older values of the element. So prioritize the passed
          // properties.
    ...
    if (current.opcode === Opcode.Insert && opcode === Opcode.Delete) {
      changedElements.delete(elementId);
      return;
    }

    // Updated element then it was deleted, accumulate as a deletion
    if (current.opcode === Opcode.Update && opcode === Opcode.Delete) {
      changedElements.set(
        elementId,
        {
          ...current,
    ...
    if (current.opcode === Opcode.Delete && opcode === Opcode.Insert) {
      changedElements.set(
        elementId,
        {
          ...current,
    ...
    if (
      (opcode === Opcode.Insert && current.opcode === Opcode.Update) ||
      // Support Insert + Insert - happens due to schema change + overflow tables
      (opcode === Opcode.Insert && current.opcode === Opcode.Insert)
    ) {
      changedElements.set(
        elementId,
        {
    ...
    if (
      (opcode === Opcode.Update && current.opcode === Opcode.Update) ||
      // Support Update + Insert - happens due to schema change + overflow tables - keep as update
      (opcode === Opcode.Update && current.opcode === Opcode.Insert)
    ) {
      // Merge type of change
      changedElements.set(
        elementId,
    ...
    if (opcode === Opcode.Insert && current.opcode === Opcode.Delete) {
      changedElements.delete(elementId);
      return;
    }

    // Update and then delete, we accumulate as a deletion
    if (opcode === Opcode.Update && current.opcode === Opcode.Delete) {
      changedElements.set(
        elementId,
        {
          ...current,
    ...
    // Delete + Insert may happen due to connectors logic, accept for now
    if (opcode === Opcode.Delete && current.opcode === Opcode.Insert) {
      changedElements.set(
        elementId,
        {
          ...current,
...
/** Merge property maps into single one and ensure that checksums are different. */
export const mergeProperties = (
  ...
  const allProps = new Set([...(newProps?.keys() ?? []), ...(oldProps?.keys() ?? [])]);
  const merged = new Map<string, Checksums>();
  for (const prop of allProps) {
    // Keep the old checksum if the new one is undefined
    const newChecksum: number | undefined = newProps?.get(prop)?.newChecksum ?? oldProps?.get(prop)?.newChecksum;
    ...
    const oldChecksum: number | undefined = oldProps?.get(prop)?.oldChecksum ?? newProps?.get(prop)?.oldChecksum;
    ...
    if ((newChecksum === undefined && oldChecksum === undefined) || newChecksum !== oldChecksum) {
      merged.set(prop, { newChecksum, oldChecksum });
    }
  }
...
export const extractProperties = (changeset: ChangedElements, index: number): Map<string, Checksums> | undefined => {
  if (changeset.opcodes[index] === Opcode.Update && changeset.properties) {
    const map = new Map<string, Checksums>();
    for (let propIndex = 0; propIndex < changeset.properties[index].length; ++propIndex) {
      const property = changeset.properties[index][propIndex];
      // Checksums may or may not be available in change data. So handle them properly for out-of-date data.
      const newChecksum = changeset.newChecksums ? changeset.newChecksums[index][propIndex] : undefined;
      const oldChecksum = changeset.oldChecksums ? changeset.oldChecksums[index][propIndex] : undefined;
      map.set(property, { newChecksum, oldChecksum });
    }
    return map;
  }
...
export const accumulateChanges = (
  ...
  changeset.elements.forEach((elementId: string, index: number) => {
    ...
    const parentId = changeset.parentIds !== undefined ? changeset.parentIds[index] : undefined;
    const parentClassId = changeset.parentClassIds !== undefined ? changeset.parentClassIds[index] : undefined;
    const modelId = changeset.modelIds !== undefined ? changeset.modelIds[index] : undefined;
...
export class ChangedElementsManager {
  ...
  private _changedElements: Map<string, ChangedElement> = new Map<string, ChangedElement>();
   ...
   */
  public async generateEntries(currentIModel: IModelConnection, targetIModel: IModelConnection): Promise<void> {
  ...
  private async _getGeometricElement3dClassId(iModel: IModelConnection): Promise<string | undefined> {
    const ecsql = `
      SELECT ECClassDef.ECInstanceId as geomId
      FROM meta.ECClassDef
      INNER JOIN meta.ECSchemaDef ON ECSchemaDef.ECInstanceId = ECClassDef.Schema.Id
      WHERE ECClassDef.Name = 'GeometricElement3d' AND ECSchemaDef.Name ='BisCore'`;
    for await (const row of iModel.query(ecsql, undefined, { rowFormat: QueryRowFormat.UseJsPropertyNames })) {
      return row.geomId;
    }
  ...
  private _getElementsInCurrent(forward: boolean): ChangedElement[] {
    const array = [...this._changedElements]
      .filter((entry) => forward ? entry[1].opcode !== Opcode.Update : entry[1].opcode !== Opcode.Delete)
      .map((entry) => entry[1]);
    return array;
  }
  ...
  private _getIds(changedElements: ChangedElement[]): string[] {
    return changedElements.map((entry) => entry.id);
  }
  ...
  /**
   * Get props for all elements and get changed models. Later on this data will be provided in the Changed Elements
   * Service.
   * @param currentIModel Current IModelConnection
   ...
   * @param progressLoadingEvent Progress Loading event that will be raised whenever progress is done finding the
   *                             changed models
   */
  private async findChangedModels(
    ...
    const steps = Math.floor(currentIds.length / chunkSize) + Math.floor(targetIds.length / chunkSize);
    const message = IModelApp.localization.getLocalizedString(
    ...
    const getModels = async (
      ...
      let ecsql = "SELECT Model as model, ECInstanceId as elemId FROM BisCore.Element WHERE ECInstanceId IN (";
      ...
      for (let i = 0; i < elementIds.length; i += chunkSize) {
        ...
        outputProgressMessage(lastStep ? lastStep + currentStep : currentStep, steps);

        const iterator = iModel.query(ecsql, QueryBinder.from(piece), { rowFormat: QueryRowFormat.UseJsPropertyNames });
    ...
    await getModels(targetIds, targetIModel, allModelIds, Math.floor(currentIds.length / chunkSize));
    return allModelIds;
  }
   ...
   */
  private async findUnchangedModels(currentIModel: IModelConnection, changedModels: Set<string>) {
    const unchangedModels = new Set<string>();
    currentIModel.models.loaded.forEach((state) => {
  ...
  private async _getModelsOfClasses(iModel: IModelConnection, modelClasses: string[]): Promise<Set<string>> {
  ...
  private async _filterChangedElementsByModelClass(
    ...
    const currentModels = await this._getModelsOfClasses(currentIModel, modelClasses);
    const targetModels = await this._getModelsOfClasses(targetIModel, modelClasses);
   ...
   */
  private _queryAndUpdateModelIds = async (iModel: IModelConnection, elementIds: string[]) => {
    let ecsql = "SELECT ECInstanceId as id, Model FROM Bis.Element WHERE ECInstanceId in (";
    elementIds.forEach(() => {
      ecsql = ecsql + "?,";
    });
    ecsql = ecsql.substr(0, ecsql.length - 1) + ")";
    const queryIterator = iModel.query(
      ecsql,
   ...
   * @param elementIds Element Ids to query for
   */
  private _queryAndUpdateModelsBulk = async (iModel: IModelConnection, elementIds: string[]) => {
    ...
    for (let i = 0; i < elementIds.length; i += chunkSize) {
      const slice = elementIds.slice(i, i + chunkSize > elementIds.length ? undefined : i + chunkSize);
      await this._queryAndUpdateModelIds(iModel, slice);
    }
  };

  /**
   * Query for the model Ids of elements that don't have it. This is a band-aid fix until we fix the addon to contain
   * deleted element model ids properly.
   */
  private _fixModelIds = async (currentIModel: IModelConnection, targetIModel: IModelConnection): Promise<void> => {
    ...
    if (currentIModel && currentElementsWithoutModels.length !== 0) {
      await this._queryAndUpdateModelsBulk(currentIModel, currentElementsWithoutModels);
    }
    if (targetIModel && targetElementsWithoutModels.length !== 0) {
      await this._queryAndUpdateModelsBulk(targetIModel, targetElementsWithoutModels);
    }
  };

  /** Whether or not the changed element data for a changeset contains type of change info. */
  private _changesetContainsTypeOfChangeData = (changeset: ChangedElements): boolean => {
    return changeset.type.some((toc) => (toc as number) !== 0);
  };

  /** Whether or not the changed element data for a changeset contains property info. */
  private _changesetContainsPropertyData = (changeset: ChangedElements): boolean => {
  ...
  /**
   * Whether or not we should do clean up of merged elements depending on the data present on the changed elements data.
   */
  private _dataAllowsCleanupOfMergedElements = (changesets: ChangedElements[]): boolean => {
    for (const changeset of changesets) {
      // If any of our changesets contain property data and type of change data we are good to do cleanup
      if (this._changesetContainsPropertyData(changeset) && this._changesetContainsTypeOfChangeData(changeset)) {
        return true;
      }
    }
  ...
  /**
   * Takes an array of ChangedElements and computes the changed elements entries. The computation is done by
   * accumulating change.
  ...
  public async setChangeSets(
    ...
    inputChangesets.forEach((changeset) => {
      accumulateChanges(this._changedElements, changeset, forward);
    });

    // Clean merged elements that resulted in properties having the same checksums. Only do this if we have proper type
    // of change data and properties.
    if (this._dataAllowsCleanupOfMergedElements(inputChangesets)) {
    ...
    if (wantedModelClasses) {
      await this._filterChangedElementsByModelClass(currentIModel, targetIModel, wantedModelClasses);
    }
    ...
    if (filterSpatial) {
      ...
      const ecsql = "SELECT SourceECInstanceId FROM meta.ClasshasAllBaseClasses WHERE TargetECInstanceId = " + geom3dId;
      const validClassIds = new Set<string>();
      for await (const row of currentIModel.query(ecsql, undefined, { rowFormat: QueryRowFormat.UseJsPropertyNames })) {
    ...
    if (this._manager.reparentModelNodes) {
      // Reparent element models to display in the widget
   ...
   */
  private _isModelSubject = (jsonProps?: string): boolean => {
    ...
    return parsedJson?.Subject?.Job?.Bridge === undefined && parsedJson?.Subject?.Model?.Type !== "Hierarchy";
  };
  ...
  private _findParentSubjectOfModels = async (
    ...
    let ecsql = `
      SELECT childsub.ECInstanceId as childId, sub.JsonProperties as jsonProps, sub.ECInstanceId as parentId
      FROM Bis.Subject sub
      JOIN Bis.Subject childsub ON sub.ECInstanceId = childsub.Parent.Id AND childsub.ECInstanceId IN (`;
    ...
    if (subjectToModelMapToQuery.size !== 0) {
      const parentSubjectToModelMap = await this._findParentSubjectOfModels(iModel, subjectToModelMapToQuery);
      // Accumulate results
  ...
  private _findSubjectsOfModels = async (
    ...
    let ecsql = `
      SELECT el.ECInstanceId as ipeId, sub.JsonProperties as jsonProps, sub.ECInstanceId as subjectId
      FROM Bis.Subject sub
      JOIN Bis.InformationPartitionElement el ON sub.ECInstanceId = el.Parent.Id AND el.ECInstanceId IN (`;
    ...
    const subjectToModelMap = new Map<string, string>();
    const subjectToModelMapToQuery = new Map<string, string>();
    const iterator = iModel.query(ecsql, QueryBinder.from(ipeIds), { rowFormat: QueryRowFormat.UseJsPropertyNames });
    for await (const row of iterator) {
    ...
    if (subjectToModelMapToQuery.size !== 0) {
      const parentSubjects = await this._findParentSubjectOfModels(iModel, subjectToModelMapToQuery);
  ...
  private _isModelPartition = (jsonProps?: string) => {
    ...
    // If we don't have specific Json Props, we have the proper model Id already
    const parsedJson = JSON.parse(jsonProps);
    return parsedJson.PhysicalPartition?.Model?.Content !== undefined
  ...
  private _findModelParentModels = async (
    ...
    let ecsql = `
      SELECT m.ECInstanceId as modelId, ipe.ECInstanceId as ipeId, ipe.JsonProperties as jsonProps
      FROM BisCore.InformationPartitionElement ipe
      JOIN BisCore.Model m ON ipe.ECInstanceId = m.ModeledElement.Id AND m.ECInstanceId IN (`;
    modelIds.forEach(() => (ecsql += "?,"));
    ecsql = ecsql.substring(0, ecsql.length - 1) + ")";

    const ipeToModel = new Map<string, string>();
    const iterator = iModel.query(ecsql, QueryBinder.from(modelIds), { rowFormat: QueryRowFormat.UseJsPropertyNames });
    for await (const row of iterator) {
      // If we don't have specific Json Props, we have the proper model Id already
  ...
  private _findParentModels = async (
    ...
    for (const pair of this._changedElements) {
      if (pair[1].modelId !== undefined) {
        if (pair[1].opcode === Opcode.Delete) {
  ...
  public async initialize(
    ...
    if (progressLoadingEvent) {
      progressLoadingEvent.raiseEvent(
        IModelApp.localization.getLocalizedString("VersionCompare:changedElementsManager.computingChangedModels"),
      );
    }
    ...
    if (progressLoadingEvent) {
      progressLoadingEvent.raiseEvent(
        IModelApp.localization.getLocalizedString("VersionCompare:changedElementsManager.computingUnchangedModels"),
      );
    }
