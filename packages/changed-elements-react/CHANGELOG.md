...
* `VersionCompareManager`: Add ability to hide side-by-side comparison toggle by setting `displaySideBySideToggle` property to `false` added to props and also added prop `manageNamedVersionsSlot` to pass a component to v2 widget VC dialog instances.
